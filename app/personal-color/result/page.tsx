...
import { DetailedPersonalColorResult, PersonalColorType, DetailedPersonalColorType } from "@/lib/personalColorAnalyzer";

// 타입별 설명 및 정보
const personalColorDescriptions: Record<PersonalColorType, { title: string, description: string }> = {
  'springWarm': {
    title: '봄 웜톤',
    description: '밝고 선명한 색상이 어울리는 봄 웜톤입니다. 노란빛이 도는 피부와 밝은 눈동자가 특징이며, 생동감 있는 컬러가 잘 어울립니다.'
  },
  'summerCool': {
    title: '여름 쿨톤',
    description: '부드럽고 연한 파스텔 색상이 어울리는 여름 쿨톤입니다. 붉은빛이 도는 피부와 차분한 눈동자가 특징이며, 부드러운 컬러가 잘 어울립니다.'
  },
  'autumnWarm': {
    title: '가을 웜톤',
    description: '깊고 풍부한 자연 색상이 어울리는 가을 웜톤입니다. 황금빛이 도는 피부와 깊은 눈동자가 특징이며, 따뜻한 컬러가 잘 어울립니다.'
  },
  'winterCool': {
    title: '겨울 쿨톤',
    description: '선명하고 대비가 강한 색상이 어울리는 겨울 쿨톤입니다. 푸른빛이 도는 피부와 강렬한 눈동자가 특징이며, 선명한 컬러가 잘 어울립니다.'
  }
};
...
const makeupTips: Record<PersonalColorType, string> = {
  ...
  'summerCool': '로즈, 라벤더 계열의 블러셔와 핑크 계열 립스틱이 잘 어울립니다. 아이섀도우는 라벤더, 그레이 블루 계열로 부드럽게 연출하세요.',
  'autumnWarm': '테라코타, 브릭 계열의 블러셔와 오렌지 브라운 립스틱이 잘 어울립니다. 아이섀도우는 브라운, 올리브 그린 계열로 깊이감 있게 연출하세요.',
  'winterCool': '푸시아, 와인 색상의 블러셔와 선명한 레드 립스틱이 잘 어울립니다. 아이섀도우는 네이비, 플럼, 그레이 계열로 또렷하게 연출하세요.'
};

// 패션 코디 팁 정의
const fashionTips: Record<PersonalColorType, string> = {
  'springWarm': '밝고 생동감 있는 컬러의 옷을 선택하세요. 연한 베이지, 산호색, 선명한 노랑이 잘 어울립니다. 골드 주얼리가 피부톤을 더욱 돋보이게 해줍니다.',
  'summerCool': '부드럽고 차분한 파스텔 컬러의 옷을 선택하세요. 라벤더, 하늘색, 부드러운 핑크가 잘 어울립니다. 실버 주얼리가 피부톤과 조화를 이룹니다.',
  'autumnWarm': '풍부하고 깊이 있는 어스 톤 컬러의 옷을 선택하세요. 카키, 머스타드, 버건디가 잘 어울립니다. 골드나 구리 톤의 주얼리가 완벽하게 어울립니다.',
  'winterCool': '선명하고 대비가 강한 컬러의 옷을 선택하세요. 퓨어 화이트, 블랙, 선명한 레드, 로열 블루가 잘 어울립니다. 플래티넘이나 실버 주얼리가 세련된 느낌을 더합니다.'
};
...
// 세부 타입별 설명 추가
const detailedDescriptions: Record<DetailedPersonalColorType, { title: string, description: string }> = {
  'brightSpring': {
    title: '선명한 봄(Bright Spring)',
    description: '생동감 있고 강렬한 색상이 어울리는 밝고 따뜻한 특성을 가진 타입입니다. 선명하고 채도 높은 컬러가 매우 잘 어울립니다.'
  },
  'lightSpring': {
    title: '라이트 봄(Light Spring)',
    description: '밝고 화사한 파스텔 톤이 잘 어울리는 봄 타입입니다. 밝고 가벼운 느낌의 색상이 피부를 환하게 만들어줍니다.'
  },
  'warmSpring': {
    title: '따뜻한 봄(Warm Spring)',
    description: '황금빛이 도는 따뜻하고 부드러운 색상이 잘 어울리는 봄 타입입니다. 복숭아색, 황금색 등이 특히 잘 어울립니다.'
  },
  'lightSummer': {
    title: '라이트 여름(Light Summer)',
    description: '밝고 부드러운 파스텔 톤이 잘 어울리는 여름 타입입니다. 연한 하늘색, 라벤더, 라일락 등이 매우 잘 어울립니다.'
  },
  'coolSummer': {
    title: '쿨 여름(Cool Summer)',
    description: '차갑고 부드러운 색상이 잘 어울리는 전형적인 여름 타입입니다. 블루 베이스의 차분한 색상이 조화롭습니다.'
  },
  'softSummer': {
    title: '소프트 여름(Soft Summer)',
    description: '부드럽고 흐릿한 파스텔 톤이 잘 어울리는 여름 타입입니다. 그레이가 섞인 듯한 부드러운 색상이 잘 어울립니다.'
  },
  'warmAutumn': {
    title: '따뜻한 가을(Warm Autumn)',
    description: '황토색, 올리브, 황금색 등 자연에서 볼 수 있는 따뜻한 색상이 잘 어울리는 가을 타입입니다.'
  },
  'deepAutumn': {
    title: '딥 가을(Deep Autumn)',
    description: '풍부하고 깊이 있는 어두운 색상이 잘 어울리는 가을 타입입니다. 버건디, 다크 그린 등이 특히 잘 어울립니다.'
  },
  'softAutumn': {
    title: '소프트 가을(Soft Autumn)',
    description: '부드럽고 따뜻한 자연 색상이 잘 어울리는 가을 타입입니다. 흙과 나무의 색상처럼 중간 톤의 색상이 잘 어울립니다.'
  },
  'coolWinter': {
    title: '쿨 겨울(Cool Winter)',
    description: '차갑고 선명한 색상이 잘 어울리는 전형적인 겨울 타입입니다. 로얄 블루, 퓨시아 등 차가운 계열의 색상이 돋보입니다.'
  },
  'deepWinter': {
    title: '딥 겨울(Deep Winter)',
    description: '깊고 강렬한 색상이 잘 어울리는 겨울 타입입니다. 보르도, 다크 네이비 등 짙은 컬러가 특히 잘 어울립니다.'
  },
  'brightWinter': {
    title: '브라이트 겨울(Bright Winter)',
    description: '강렬하고 선명한 색상이 잘 어울리는 겨울 타입입니다. 선명한 레드, 로얄 블루 등 채도 높은 색상이 잘 어울립니다.'
  }
};

export default function ResultPage() {
  const router = useRouter();
  const [result, setResult] = useState<DetailedPersonalColorResult | null>(null);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-teal-50/70 via-slate-50/70 to-white">
        <div className="bg-white/60 backdrop-blur-xl rounded-3xl shadow-2xl p-8 max-w-md text-center">
          <h2 className="text-xl font-bold text-slate-800 mb-4">분석 오류 발생</h2>
  ...
  if (!result) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-teal-50/70 via-slate-50/70 to-white">
        <div className="bg-white/60 backdrop-blur-xl rounded-3xl shadow-2xl p-8 max-w-md text-center">
          <h2 className="text-xl font-bold text-slate-800 mb-4">결과를 불러올 수 없습니다</h2>
          <p className="text-slate-600 mb-6">분석 결과를 찾을 수 없습니다. 다시 분석을 시도해주세요.</p>
          <button
            onClick={() => router.push('/personal-color/capture')}
            className="px-6 py-3 bg-gradient-to-r from-teal-500 to-cyan-500 text-white font-medium rounded-xl shadow-md hover:shadow-lg transition-shadow"
          >
  ...
  const bgGradient = backgroundGradients[resultType] || 'from-teal-50/70 via-slate-50/70 to-white';
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <div className="mt-4 inline-flex items-center bg-white/70 backdrop-blur-sm px-4 py-1.5 rounded-full shadow-sm">
              <span className="text-sm font-medium text-teal-700">분석 신뢰도: {result.confidence}% ({getConfidenceText(result.confidence)})</span>
            </div>
          </motion.div>
              ...
              <div className="md:flex">
                ...
                <div className="md:w-3/5 p-6 md:p-8">
                  <div className="mb-6">
                    ...
                    <div className="bg-slate-50/70 backdrop-blur-sm rounded-2xl p-5 shadow-sm border border-white/50">
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-sm font-medium text-slate-700">{detailedTitle}</span>
                        <span className={`text-xs font-medium px-3 py-1 rounded-full shadow-sm ${
                          result.tone === 'warm' ? 'bg-amber-50 text-amber-700 border border-amber-100' : 'bg-blue-50 text-blue-700 border border-blue-100'
                        }`}>
                          {result.tone === 'warm' ? '웜톤' : '쿨톤'}
                        </span>
                      </div>
                      <p className="text-sm text-slate-600">{detailedDescription}</p>
                    </div>
                  </div>
                  
                  {/* 피부 특성 */}
                  <div className="mb-6">
                    <h3 className="text-lg font-bold text-slate-800 mb-3">피부 특성</h3>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="bg-slate-50/70 backdrop-blur-sm p-3.5 rounded-2xl shadow-sm border border-white/50">
                        ...
                        <div className="text-sm font-medium text-slate-700">
                          {skinBrightness === 'light' ? '밝은 톤' : 
                           skinBrightness === 'deep' ? '어두운 톤' : '중간 톤'}
                        </div>
                      </div>
                      <div className="bg-slate-50/70 backdrop-blur-sm p-3.5 rounded-2xl shadow-sm border border-white/50">
                        <div className="text-xs text-slate-500 mb-1">청탁도</div>
                        <div className="text-sm font-medium text-slate-700">
                          {skinClarity === 'clear' ? '선명한 톤' : 
                           skinClarity === 'muted' ? '흐린 톤' : '중간 톤'}
                        </div>
                      </div>
                      <div className="bg-slate-50/70 backdrop-blur-sm p-3.5 rounded-2xl shadow-sm border border-white/50">
                        <div className="text-xs text-slate-500 mb-1">언더톤</div>
                        <div className="text-sm font-medium text-slate-700">
                          {result.colorValues.skinUndertone === 'yellow' ? '황색 베이스' : 
                           result.colorValues.skinUndertone === 'pink' ? '붉은 베이스' : '중성 베이스'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-5 bg-slate-50/70 backdrop-blur-sm p-4 rounded-2xl shadow-sm border border-white/50">
                      <div className="text-xs text-slate-500 mb-3">주요 피부색 팔레트</div>
                  ...
                  <div className="mb-6 bg-slate-50/70 backdrop-blur-sm p-5 rounded-2xl shadow-sm border border-white/50">
                    <h4 className="text-sm font-medium text-slate-700 mb-3">주요 특징</h4>
                  ...
                  {/* 컬러 분석 정보 */}
                  <div className="mb-6">
                    <h4 className="text-sm font-medium text-slate-700 mb-3">색상 분석 정보</h4>
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="bg-slate-50/70 backdrop-blur-sm p-3 rounded-xl shadow-sm border border-white/50">
                        ...
                        <span className="ml-1 font-medium text-slate-700">{Math.round(result.colorMetrics.ita)} ({getITADescription(result.colorMetrics.ita)})</span>
                      </div>
                      <div className="bg-slate-50/70 backdrop-blur-sm p-3 rounded-xl shadow-sm border border-white/50">
                        <span className="text-slate-500">색온도:</span>
                        <span className="ml-1 font-medium text-slate-700">{Math.round(result.colorMetrics.temperature)} ({getTemperatureDescription(result.colorMetrics.temperature)})</span>
                      </div>
                      <div className="bg-slate-50/70 backdrop-blur-sm p-3 rounded-xl shadow-sm border border-white/50">
                        <span className="text-slate-500">대비:</span>
                        <span className="ml-1 font-medium text-slate-700">{Math.round(result.colorMetrics.contrast)} ({result.colorValues.contrastLevel})</span>
                      </div>
                      <div className="bg-slate-50/70 backdrop-blur-sm p-3 rounded-xl shadow-sm border border-white/50">
                        <span className="text-slate-500">밝기:</span>
                        <span className="ml-1 font-medium text-slate-700">{Math.round(result.colorMetrics.brightness)}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-6 md:p-8 border-t border-slate-100/30 bg-slate-50/30 backdrop-blur-sm">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-white/60 backdrop-blur-sm p-5 rounded-2xl shadow-sm border border-white/60">
                    <h4 className="text-sm font-medium text-slate-700 mb-4">추천 컬러 팔레트</h4>
                  ...
                  <div className="bg-white/60 backdrop-blur-sm p-5 rounded-2xl shadow-sm border border-white/60">
                    <h4 className="text-sm font-medium text-slate-700 mb-4">피해야 할 색상</h4>
                    <div className="grid grid-cols-5 gap-3">
              ...
              <div className="relative">
                ...
                {showShareOptions && (
                  <div className="absolute bottom-full right-0 mb-2 bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl p-2 z-10 border border-white/30 w-[280px]">
                    ...
                    >
                      <svg className="w-4 h-4 text-slate-600 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {/* 카카오톡 공유 버튼 */}
                    <KakaoShareButton 
                      title={`MySkinMetrix - ${resultTitle}`}
                      description={`내 퍼스널 컬러는 ${resultTitle}(${detailedTitle})입니다! MySkinMetrix에서 나의 퍼스널 컬러를 확인해보세요.`}
                      imageUrl={imageUrl || "https://myskinmetrix.vercel.app/images/og_image.jpg"}
                    ...
                    <ResultShareCard 
                      ...
                      downloadFileName={`myskinmetrix-${resultType}-result.png`}
                    />
                  </div>
                )}
              </div>
